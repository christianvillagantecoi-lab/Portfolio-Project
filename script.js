...
function setupScrollRevealAnimations() {
  ...
  pageViews.forEach((view) => {
    ...
    viewNodes.forEach((node, index) => {
      ...
      if (node.matches('.resume-grid-layout, .resume-sidebar, .resume-content-area, .resume-photo-container, .resume-avatar, .resume-education-accent, .home-parallax-container, .home-parallax-media, .home-parallax-shadow')) return;
...
window.addEventListener('scroll', () => {
  ...
  const homeMedia = document.querySelector('.home-parallax-media');
  const scrollPos = window.pageYOffset;
  if (banner) {
    banner.style.setProperty('--parallax-scroll-y', `${scrollPos * 0.4}px`);
  }
  if (homeMedia) {
    homeMedia.style.setProperty('--home-parallax-scroll-y', `${scrollPos * 0.18}px`);
  }
});
